/*
---------------------------------------------
ALARMA CON REED SWITCH, LED RGB y BUZZER/ZUMBADOR
---------------------------------------------
...
//--------------------------------------------------
int buzzer = 5; //Declaracion del pin que contiene al Buzzer
int frecuencia = 220; //Frecuencia a la que trabajara el buzzer
...
void setup() { //Se ejecuta cuando el arduino inicia
  ...
  pinMode(buzzer,OUTPUT);   //Se activa la alarma y su salida es el buzzer
...
if (digitalRead(contacto) == LOW) { //Cuando el Reed Switch esta en pasivo
    ...
    tone(buzzer,frecuencia);      //El buzzer toma el valor de la frecuencia y comienza a sonar
      color (255, 0, 0); //Rojo
      delay(50);
      color (0, 0, 0); //Apagado
      delay(50);
